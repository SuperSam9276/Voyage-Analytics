...
# load data (resolved relative to this script, not the caller's working directory)
DATA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "users_cleaned.csv")
...
# derive a gender signal from first name before dropping the name column.
# company/age carry ~0 mutual information with gender in this dataset (verified
# via CV: ~50% accuracy, same as the majority-class baseline) - name is the
# only column that actually correlates with the target.
detector = gender_detector.Detector(case_sensitive=False)
df["first_name"] = df["name"].str.split().str[0]
df["name_guess"] = df["first_name"].apply(detector.get_gender)

# drop non-predictive identifier columns
df = df.drop(columns=["code", "name", "first_name"])


# Turns the gender text (male/female/etc.) into numbers (0, 1, 2...) since ML models need numbers, not text. Prints which number maps to which gender.
le = LabelEncoder()
...
# encode features
X = pd.get_dummies(df.drop(columns=["gender"]), columns=["company", "name_guess"], drop_first=True)
